...
!!! Note
    ...
    Using a code provider is optional if you don't want to provide skill sources to a code action model.
...
For obtaining only the final response, await `response()` on a [`CodeActAgentTurn`][freeact.agent.CodeActAgentTurn] returned by the [`CodeActAgent.run`][freeact.agent.CodeActAgent.run] method. It waits until the sequence of model interactions and code executions driven by the agent is complete.
...
4. Accumulate token usage and costs across multiple agent runs in a conversation. See also [usage statistics](#usage-statistics).

## Usage statistics

[`CodeActModelResponse.usage`][freeact.model.CodeActModelResponse.usage] provides token usage and costs for obtaining a single model response, [`CodeActAgentResponse.usage`][freeact.agent.CodeActAgentResponse.usage] provides accumulated token usage and costs for a single agent response. For accumulating token usage and costs across agent responses in a conversation, create an instance of [`CodeActModelUsage`][freeact.model.CodeActModelUsage] and call its [`update`][freeact.model.CodeActModelUsage.update] method with agent response `usage` objects.
