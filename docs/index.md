...
| Feature | Description |
|---------|-------------|
| **Programmatic tool calling** | Agents call tools programmatically within code actions rather than through JSON structures. Freeact auto-generates typed Python modules from MCP tool schemas to enable this. LLMs are heavily pretrained on Python code, making this more reliable than JSON tool calling. |
...
| **Agent skills** | Freeact supports the [agentskills.io](https://agentskills.io/) specification - a lightweight format for extending agent capabilities with specialized knowledge and workflows. Freeact provides builtin skills for saving code actions as tools to persistent memory, augmenting existing tools, and structured task planning. |
| **Progressive loading** | Tool and skill information is loaded in stages as needed, rather than consuming context upfront. This solves a scaling problem: preloading large collections wastes tokens on capabilities irrelevant for the current task. For tools: category names, tool names, and API definitions load progressively as needed. For skills: metadata loads at startup; full instructions load when triggered. |
| **Sandboxed execution** | Code actions execute locally in a stateful [IPython](https://github.com/ipython/ipython) kernel via [ipybox](https://gradion-ai.github.io/ipybox/). Optional sandboxing via [sandbox-runtime](https://github.com/anthropic-experimental/sandbox-runtime) restricts filesystem access (e.g., block `.env` reads) and network access (e.g., allow only specific domains). Stdio MCP servers can be sandboxed independently. |
| **Unified approval** | Code actions, programmatic tool calls, and JSON-based tool calls all flow through a unified approval mechanism that yields requests for inspection before proceeding. A single approval flow ensures every action can be inspected and gated regardless of how it originates. |
