...
- `create_user_input_box(content)` -> `Collapsible`
...
- `create_error_box(message)` -> `Collapsible`
- `create_file_read_action_box(tool_name, paths, head, tail, agent_id, corr_id)` -> `Collapsible`
