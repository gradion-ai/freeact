...
Freeact executes Python code and shell commands in an IPython kernel provided by [ipybox](https://github.com/gradion-ai/ipybox). Both run through the same `ipybox_execute_ipython_cell` [internal tool](sdk.md#internal-tools), and the kernel is stateful: variables, imports, and function definitions persist across executions within a session.
