...
class LiteCodeActModel(CodeActModel):
    ...
    - `use_executor_tool=False`: Code actions are included directly into the model's response text,
      enclosed in `<code-action> ... </code-action>` tags. This instruction, among others, is given
      by the [`CODE_TAG_SYSTEM_TEMPLATE`][freeact.model.CODE_TAG_SYSTEM_TEMPLATE].
    ...
    - `use_executor_tool=True`: Code actions are generated by calling an internal `execute_ipython_cell`
      ...
      [`TOOL_USE_SYSTEM_TEMPLATE`][freeact.model.TOOL_USE_SYSTEM_TEMPLATE].
    ...
    Its semantics should then match the `use_executor_tool` argument value.
