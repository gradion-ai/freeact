...
class LiteCodeActModel(CodeActModel):
    ...
    - `use_executor_tool=False`: Code actions are included directly into the model's response text,
      enclosed in `<code-action> ... </code-action>` tags. This instruction is given by the
      [`CODE_TAG_SYSTEM_TEMPLATE`][freeact.model.CODE_TAG_SYSTEM_TEMPLATE].

    - `use_executor_tool=True`: Code actions are generated by calling an internal `execute_ipython_cell`
      tool. This instruction is given by the [`TOOL_USE_SYSTEM_TEMPLATE`][freeact.model.TOOL_USE_SYSTEM_TEMPLATE].
