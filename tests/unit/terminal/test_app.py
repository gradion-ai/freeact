...
async def _response_scenario(_: PromptContent) -> AsyncIterator[AgentEvent]:
...
def _patch_banner(monkeypatch: pytest.MonkeyPatch, banner: Text) -> None:
    monkeypatch.setattr("freeact.terminal.app._load_banner", lambda: banner)
...
async def test_toggle_expand_all_uses_configured_hotkey() -> None:
    app = create_app(MockStreamAgent(no_events).stream, config=TerminalConfig(expand_all_toggle_key="f6"))
...
async def test_approval_behaviors_can_disable_auto_collapse(
    ...
    ui_config = TerminalConfig(collapse_approved_tool_calls=False, keep_rejected_actions_expanded=False)
    app = create_app(
        MockStreamAgent(approval_scenario(db_query_call())).stream,
        config=ui_config,
        permission_manager=StubPermissionManager(preapproved=True) if preapproved else None,
    )
...
async def test_approved_code_and_tool_collapse_behaviors_are_independent(
    ...
    ui_config = TerminalConfig(collapse_approved_code_actions=False, collapse_approved_tool_calls=True)
...
@pytest.mark.parametrize(
    ...
    [
        (code_action_call(), ".code-action-box", TerminalConfig(collapse_approved_code_actions=False)),
        (db_query_call(), ".tool-call-box", TerminalConfig(collapse_approved_tool_calls=False)),
    ],
)
async def test_preapproved_action_respects_collapse_config(
    tool_call: ToolCall, box_selector: str, ui_config: TerminalConfig
) -> None:
    app = create_app(
...
@pytest.mark.parametrize(
    ...
    [(None, True), (TerminalConfig(collapse_completed_subagent_tasks=False), False)],
)
async def test_completed_subagent_task_auto_collapse_is_configurable(
    ui_config: TerminalConfig | None, expected_collapsed: bool
) -> None:
    async def scenario(_: PromptContent) -> AsyncIterator[AgentEvent]:
        task_request = approval_request(_subagent_task_call("delegate"), "task-1")
        yield task_request
